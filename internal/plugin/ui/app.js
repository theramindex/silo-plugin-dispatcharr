...
function mergePrefs(remote) {
  ...
  return {
    ...
    sportsSpoilersHidden: remote.sportsSpoilersHidden === true,
    sportsPlayerSpoilersHidden: remote.sportsPlayerSpoilersHidden === true,
    featuredEvents: Object.assign({}, remote.featuredEvents),
...
function normalizePreferences() {
  ...
  state.app.preferences.sportsSpoilersHidden = state.app.preferences.sportsSpoilersHidden === true;
  state.app.preferences.sportsPlayerSpoilersHidden = state.app.preferences.sportsPlayerSpoilersHidden === true;
  state.app.preferences.featuredEvents = state.app.preferences.featuredEvents || {};
...
function normalizeAdminCategorySettings() {
  ...
  const seenTeamPins = {};
  state.adminCategorySettings.sportsTeamChannels = items(state.adminCategorySettings.sportsTeamChannels).map(function(pin) {
    return { leagueId: String((pin && pin.leagueId) || "").trim(), teamName: String((pin && pin.teamName) || "").trim(), channelId: String((pin && pin.channelId) || "").trim(), channelName: String((pin && pin.channelName) || "").trim() };
  }).filter(function(pin) {
    const key = pin.leagueId + "|" + lower(pin.teamName);
    if (!pin.leagueId || !pin.teamName || !pin.channelId || seenTeamPins[key]) return false;
    seenTeamPins[key] = true;
    return true;
  }).slice(0, 200);
  state.adminCategorySettings.sportsFirstPlayerEnabled = state.adminCategorySettings.sportsFirstPlayerEnabled === true;
  state.adminCategorySettings.featuredEventIds = uniqueIDs(items(state.adminCategorySettings.featuredEventIds).map(function(id) {
...
async function corePostJSON(url, body) {
  const response = await coreFetch(url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
  if (!response.ok) throw await requestError(response);
  return response.json();
}
...
function visibleChannels(ignoreQuery) {
  ...
  const channels = dedupeChannels(effectiveChannels(false).filter(function(channel) {
...
function appRouteSnapshot() {
  return {
    ...
    sportsTab: state.sportsTab || "today",
    sportsLeague: state.sportsLeague || "",
    sportsTeam: state.sportsTeam || "",
...
function applyAppRouteSnapshot(snapshot) {
  ...
  state.sportsTab = snapshot.sportsTab || "today";
  state.sportsLeague = snapshot.sportsLeague || "";
  state.sportsTeam = snapshot.sportsTeam || "";
...
async function refreshSupplementalData() {
  if (!state.app) return;
  const payload = await getJSON("/dispatcharr/api/guide").catch(function(error) {
    try { console.warn("Dispatcharr guide load failed", error); } catch (_) {}
    return null;
  });
  if (payload) state.app.programs = items(payload.programs);
  rebuildProgramIndex();
...
function programLooksSports(program) {
  if (programIsGuidePlaceholder(program)) return false;
  const channel = channelByID(program.channelId) || {};
  const text = [program.title, channel.name, channel.categoryName].join(" ");
  return /\b(vs\.?| at |cup|league|racing|grand prix|football|baseball|basketball|soccer|hockey|golf|tennis|boxing|mma|ufc|f1|formula|mlb|nba|nfl|nhl|mls|wnba|premier|sports?)\b/i.test(" " + text + " ");
}
...
function programLooksEvent(program) {
  if (programIsGuidePlaceholder(program)) return false;
  return /\b(awards|parade|special|ceremony|debate|concert|festival|inauguration|election night)\b/i.test(String(program.title || ""));
}
...
function searchResultSections(query) {
  ...
  if (include("movies")) {
    const guideMovies = firstSearchMatches(programs(), function(program) { return programLooksMovie(program) && programMatchesSearch(program, query); }, 12);
    sections.push({ id: "movies", title: "Movies From Guide", rows: guideMovies.map(function(program) {
      const channel = channelByID(program.channelId) || {};
      return {
        attrs: "data-search-program-channel=\"" + escapeHTML(program.channelId || "") + "\" data-search-program=\"" + escapeHTML(program.id || "") + "\"",
        art: logoHTML(channel),
...
function myTVFollowedPeople() {
  ...
  Object.keys(hubTeams).forEach(function(key) {
    const entry = hubTeams[key];
    items(entry.team && entry.team.followIds).forEach(function(followID) {
      if (!people[followID]) people[followID] = Object.assign({ kind: "Team", leagueName: entry.leagueName }, entry.team, { id: followID });
    });
  });
...
function myTVFollowingHTML() {
  ...
  const named = people.filter(function(team) { return !team.unresolved; });
  const unnamed = people.length - named.length;
  const cards = searches.concat(named.map(function(team) { return myTVFollowingCard("team", team); }), leagues.map(function(league) { return myTVFollowingCard("league", league); }));
  const total = cards.length + unnamed;
  const unnamedNote = unnamed ? "<p class=\"my-tv-follow-note\">" + escapeHTML(pluralLabel(unnamed, "more team") + " will appear when they're back on the schedule.") + "</p>" : "";
  if (!cards.length && !unnamedNote) return "";
  return "<section class=\"my-tv-section\" aria-label=\"Following\"><header><h3>Following</h3><span>" + escapeHTML(pluralLabel(total, "follow")) + "</span></header>" + (cards.length ? "<div class=\"my-tv-following\">" + cards.join("") + "</div>" : "") + unnamedNote + "</section>";
}
...
function myTVWatchedOftenHTML() {
  ...
  if (!channels.length) return "";
  return '<section class="my-tv-section my-tv-favorites" aria-label="Channels you watch often"><header><h3>Watched often</h3><span>Tap the heart to keep one as a favorite</span></header><div class="search-result-list">' + channels.map(myTVChannelRow).join('') + '</div></section>';
}
function myTVDashboardHTML() {
  ...
  return myTVFavoriteChannelsHTML() + myTVWatchedOftenHTML() + myTVFollowingHTML() + (upcoming ? "<section class=\"my-tv-up-next\"><header><h3>Up next</h3></header>" + upcoming + "</section>" : "") + empty;
}
...
function loadSportsLibraries(force) {
  ...
  state.sportsLibrariesPromise = coreGetJSON(siloCoreURL("/user/libraries")).then(function(payload) {
    ...
    return state.sportsLibraries;
  }).catch(function(error) {
    state.sportsLibrariesError = readableError(error);
    if (!state.sportsLibraries) state.sportsLibraries = [];
    return state.sportsLibraries;
  }).finally(function() {
    state.sportsLibrariesLoading = false;
    state.sportsLibrariesPromise = null;
    if (state.view === "admin" && state.adminTab === "sports") renderAdminPage();
  });
  return state.sportsLibrariesPromise;
}
function accessibleSportsLibraryIDs(libraries) {
  const matcher = window.SportsReplayMatcher;
  if (matcher && typeof matcher.accessibleLibraryIDs === "function") return matcher.accessibleLibraryIDs(configuredSportsLibraryIDs(), libraries);
  const accessible = {};
  items(libraries).forEach(function(library) { accessible[Number(library.id)] = true; });
  return configuredSportsLibraryIDs().filter(function(id) { return !!accessible[id]; });
}
async function loadSportsReplayLibrary(libraryID, catalogWindows) {
  const catalogItems = [];
  let truncated = false;
  const collector = window.SportsReplayMatcher;
...
function dedupeSportsReplayItems(catalogItems) {
  const seen = {};
  return items(catalogItems).filter(function(item) {
    const id = String(item && item.content_id || "");
    if (!id || seen[id]) return false;
    seen[id] = true;
    return true;
  });
}
function buildSportsReplayMatches(events, replayItems) {
  const matcher = window.SportsReplayMatcher;
  return matcher && typeof matcher.matchEvents === "function"
    ? matcher.matchEvents(events, replayItems)
    : {};
}
...
function loadSportsReplays(force) {
  ...
  if (state.sportsReplayPromise) {
    state.sportsReplayRefreshQueued = state.sportsReplayRefreshQueued || !!force;
    return state.sportsReplayPromise;
  }
  ...
  const promise = loadSportsLibraries(force).then(function(libraries) {
    ...
    const catalogWindows = sportsReplayCatalogWindows(eventSnapshot);
    const eventIDs = eventSnapshot.map(function(event) { return event.id; }).join(",");
    const key = libraryIDs.join(",") + "|" + eventIDs + "|" + catalogWindows.map(function(window) { return window.join(":"); }).join(",");
    if (!force && key && key === state.sportsReplayKey) return state.sportsReplayMatches;
    if (!libraryIDs.length || !catalogWindows.length) {
    ...
    return Promise.all(libraryIDs.map(function(libraryID) {
      return loadSportsReplayLibrary(libraryID, catalogWindows).then(function(result) {
      ...
      if (generation !== state.sportsReplayGeneration) return state.sportsReplayMatches;
      const successful = results.filter(function(result) { return result.ok; });
      const failed = results.filter(function(result) { return !result.ok; });
      const truncated = successful.filter(function(result) { return result.truncated; });
    ...
    if (generation === state.sportsReplayGeneration) {
      state.sportsReplaysError = readableError(error);
    }
    return state.sportsReplayMatches;
  }).finally(function() {
    if (generation === state.sportsReplayGeneration) {
      const refreshQueued = state.sportsReplayRefreshQueued;
      state.sportsReplaysLoading = false;
      state.sportsReplayPromise = null;
      state.sportsReplayRefreshQueued = false;
      if (state.view === "sports") renderSportsPage();
      if (refreshQueued) loadSportsReplays(true);
    }
  });
...
function loadSports(force, preparedOnly) {
  ...
  return getJSONWithin("/dispatcharr/api/sports" + (force && !preparedOnly ? "?refresh=1" : ""), 12000, "Sports data took too long to respond. Try again.").then(function(payload) {
    ...
    renderRail();
    if (state.view === "sports") renderSportsPage();
    if (state.view === "admin" && state.adminTab === "sports") renderAdminSportsSettings();
    if (state.view === "mytv") {
      updateMyTVSearchSurface();
...
function sportsTabLabel(tab) {
  return sportsHubTabLabel(sportsHubTab(tab));
}
function sportsTabButtonsHTML() {
  const current = sportsHubTab(state.sportsTab);
  return sportsHubTabs().map(function(tab) {
    return "<button type=\"button\" data-sports-tab=\"" + tab + "\" class=\"" + (current === tab ? "active" : "") + "\" aria-pressed=\"" + (current === tab ? "true" : "false") + "\">" + escapeHTML(sportsTabLabel(tab)) + "</button>";
  }).join("");
}
...
function sportsHasGameStats(event) {
  const leagues = ["mlb", "nfl", "nba", "wnba", "nhl", "mls", "premier-league", "uefa-champions-league", "college-football", "mens-college-basketball", "womens-college-basketball"];
  return !!event && (leagues.indexOf(String(event.leagueId || "")) !== -1 || /^(college football|ncaa football|mlb|major league baseball)$/i.test(event.leagueName || ""));
}
...
function renderSportsBaseballSituation(situation) {
  if (!situation) return "";
  const dots = function(count, total, label) {
    let html = "";
    for (let index = 0; index < total; index++) html += "<i class=\"" + (index < count ? "on" : "") + "\"></i>";
    return "<span class=\"sports-count\"><b>" + label + "</b>" + html + "</span>";
  };
  const base = function(on, name) { return "<span class=\"sports-base sports-base-" + name + (on ? " on" : "") + "\"></span>"; };
  const player = function(role, value) {
    if (!value) return "";
    return "<span class=\"sports-situation-player\">" + (value.headshot ? "<img src=\"" + escapeHTML(value.headshot) + "\" alt=\"\" loading=\"lazy\" onerror=\"this.remove()\">" : "") + "<span><small>" + role + "</small><strong>" + escapeHTML(value.name) + "</strong>" + (value.summary ? "<em>" + escapeHTML(value.summary) + "</em>" : "") + "</span></span>";
  };
  const runners = [situation.onFirst ? "first" : "", situation.onSecond ? "second" : "", situation.onThird ? "third" : ""].filter(Boolean);
  const label = "Balls " + situation.balls + ", strikes " + situation.strikes + ", " + situation.outs + " out" + (situation.outs === 1 ? "" : "s") + ", " + (runners.length ? "runners on " + runners.join(" and ") : "bases empty");
  return "<div class=\"sports-situation\" role=\"img\" aria-label=\"" + escapeHTML(label) + "\"><span class=\"sports-diamond\" aria-hidden=\"true\">" + base(situation.onSecond, "second") + base(situation.onThird, "third") + base(situation.onFirst, "first") + "</span>"
    + "<span class=\"sports-counts\" aria-hidden=\"true\">" + dots(situation.balls, 4, "B") + dots(situation.strikes, 3, "S") + dots(situation.outs, 3, "O") + "</span>"
    + player("At bat", situation.batter) + player("Pitching", situation.pitcher) + "</div>";
}
function renderSportsProbables(event, probables) {
  const list = items(probables);
  if (!list.length) return "";
  return "<div class=\"sports-probables\"><h3>Probable pitchers</h3>" + list.map(function(player) {
    const team = player.side === "home" ? event.home : event.away;
    return "<span class=\"sports-situation-player\">" + (player.headshot ? "<img src=\"" + escapeHTML(player.headshot) + "\" alt=\"\" loading=\"lazy\" onerror=\"this.remove()\">" : "") + "<span><small>" + escapeHTML(sportsTeamName(team || {})) + "</small><strong>" + escapeHTML(player.name) + "</strong>" + (player.summary ? "<em>" + escapeHTML(player.summary) + "</em>" : "") + "</span></span>";
  }).join("") + "</div>";
}
function renderSportsLeaders(event, leaders) {
  const list = items(leaders);
  if (!list.length) return "";
  return "<div class=\"sports-leaders\"><h3>Game leaders</h3><div>" + list.map(function(leader) {
    const team = leader.side === "home" ? event.home : (leader.side === "away" ? event.away : null);
    return "<span class=\"sports-leader\"><small>" + escapeHTML(leader.label) + "</small><strong>" + escapeHTML(leader.name) + "</strong><em>" + escapeHTML([leader.value, team ? sportsTeamAbbreviation(team) : ""].filter(Boolean).join(" · ")) + "</em></span>";
  }).join("") + "</div></div>";
}
function renderSportsPlays(plays) {
  const list = items(plays);
  if (!list.length) return "";
  return "<div class=\"sports-plays\"><h3>Recent plays</h3><ol>" + list.map(function(play) {
    const when = [play.period, play.clock].filter(Boolean).join(" · ");
    return "<li class=\"" + (play.scoring ? "scoring" : "") + "\">" + (when ? "<small>" + escapeHTML(when) + "</small>" : "") + "<span>" + escapeHTML(play.text) + "</span></li>";
  }).join("") + "</ol></div>";
}
function renderSportsHighlights(videos) {
  const list = items(videos);
  if (!list.length) return "";
  return sportsSectionHTML("Highlights", "", "<div class=\"sports-highlights\">" + list.map(function(video) {
    const thumb = "<span class=\"sports-highlight-thumb\">" + (video.thumbnail ? "<img src=\"" + escapeHTML(video.thumbnail) + "\" alt=\"\" loading=\"lazy\" onerror=\"this.remove()\">" : "") + icon("play") + "</span><strong>" + escapeHTML(video.title) + "</strong>";
    if (/^https:\/\//i.test(String(video.stream || ""))) {
      return "<button type=\"button\" class=\"sports-highlight\" data-sports-highlight-src=\"" + escapeHTML(video.stream) + "\" data-sports-highlight-title=\"" + escapeHTML(video.title) + "\" data-sports-highlight-page=\"" + escapeHTML(video.url || "") + "\">" + thumb + "</button>";
    }
    return "<a class=\"sports-highlight\" href=\"" + escapeHTML(video.url) + "\" target=\"_blank\" rel=\"noopener noreferrer\">" + thumb + "</a>";
  }).join("") + "</div>", "sports-highlights-section");
}
...
function renderSportsPage() {
  ...
  if (!state.sports && !state.sportsLoading) loadSports(false);
  if (state.sports && configuredSportsLibraryIDs().length && !state.sportsReplayKey && !state.sportsReplaysLoading && !state.sportsReplaysError) loadSportsReplays(false);
  renderSportsTopbarTabs();
  ...
  else if (state.sportsTeam) content = renderSportsTeamPage(payload, state.sportsTeam);
  else if (league) content = renderSportsLeagueDetail(payload, league, events);
  else if (sportsHubTab(state.sportsTab) === "replays") content = renderSportsBrowse(payload, events);
  else content = renderSportsHub(payload);
...
function renderSportsTileAvailability(event) {
  ...
  return "<div class=\"sports-event-availability" + (expanded ? " expanded" : "") + "\"><span>" + escapeHTML(parts.join(" · ") || "Not on your channels") + "</span>" + control + "</div>" + tray;
}
...
function renderSportsLeagueDetail(payload, league, events) {
  ...
  return "<div class=\"sports-pinned sports-detail-toolbar\"><button type=\"button\" class=\"sports-back\" data-sports-back=\"browse\">" + icon("arrow-left") + "<span>" + escapeHTML(sportsTabLabel(state.sportsTab)) + "</span></button>" + renderSportsTabFilters(payload) + "</div>"
    ...
    + renderSportsStandings(league.id)
    + sportsLeagueNewsHTML(league.id) + "</div>";
}
...
function renderSportsEventDetail(payload, event) {
  ...
  return '<div class="sports-pinned sports-detail-toolbar sports-event-toolbar">' + navigation + '<div class="sports-detail-actions"><button type="button" class="sports-detail-tool' + (sportsScoresHidden(false) ? ' active' : '') + '" data-sports-spoilers="global" aria-pressed="' + (sportsScoresHidden(false) ? 'true' : 'false') + '">' + icon(sportsScoresHidden(false) ? 'eye-off' : 'eye') + '<span>' + (sportsScoresHidden(false) ? 'Show scores' : 'Hide scores') + '</span></button>' + leagueAction + '<button type="button" class="sports-detail-tool sports-refresh" data-sports-refresh="true">' + icon("loader") + '<span>Refresh scores</span></button></div></div>'
    ...
    + '<div class="sports-event-hero-copy">' + (leagueLabel ? '<span class="sports-eyebrow">' + escapeHTML(leagueLabel) + '</span>' : '') + '<h1>' + escapeHTML(sportsEventTitle(event)) + '</h1>' + metadataHTML + renderSportsDetailScore(event) + (broadcasts ? '<div class="sports-hero-feeds sports-broadcast-section" role="group" aria-label="Watch">' + broadcasts + '</div>' : '') + (matches[0] ? '<div class="sports-feature-actions"><a class="sports-secondary-action" href="' + escapeHTML(sportsReplayHref(matches[0].item || {})) + '">' + icon("play") + '<span>Watch replay</span></a></div>' : '') + '</div></header>'
...
function sportsReplayItemTitle(item) {
  return item && (item.title || item.name || item.display_name) || "Sports replay";
}
...
function sportsReplayArtwork(item, kind) {
  ...
  const art = item.art || {};
  const backdrop = [item.backdrop_url, item.backdropUrl, item.backdrop, images.backdrop, art.backdrop, item.thumbnail_url, item.thumbnailUrl, item.image_url, item.imageUrl];
  const poster = [item.poster_url, item.posterUrl, item.poster, images.poster, art.poster, item.thumbnail_url, item.thumbnailUrl, item.image_url, item.imageUrl];
  const candidates = kind === "poster" ? poster : backdrop;
  for (let index = 0; index < candidates.length; index += 1) {
    const url = safeSportsMediaURL(candidates[index]);
    if (url) return url;
  }
  return "";
}
...
function sportsEventArtwork(event, kind) {
  ...
  return match ? sportsReplayArtwork(match.item, kind) : "";
}
function renderSportsCoverageCard(match) {
  const item = match.item || {};
  const art = sportsReplayArtwork(item, "poster");
  return "<a class=\"sports-coverage-card" + (art ? " has-art" : " no-art") + "\" href=\"" + escapeHTML(sportsReplayHref(item)) + "\">" + (art ? "<img src=\"" + escapeHTML(art) + "\" alt=\"\">" : "<span class=\"sports-coverage-placeholder\">" + icon("play") + "</span>") + "<span class=\"sports-coverage-copy\"><small>" + escapeHTML(sportsReplayDate(item)) + "</small><strong>" + escapeHTML(sportsReplayItemTitle(item)) + "</strong><b>Watch replay</b></span></a>";
}
...
function compareSportsEventsForTab(left, right) {
  ...
  const rightFollowed = sportsEventIsFollowed(right);
...
function sportsReplayMatchesForEvent(event) {
  return items(state.sportsReplayMatches && state.sportsReplayMatches[event && event.id]).filter(function(match) {
    return match && match.confidence === "high";
  });
}
function sportsReplayHref(item) {
  const id = encodeURIComponent(String(item && item.content_id || ""));
  return (item && (item.type === "movie" || item.type === "episode") ? "/watch/" : "/item/") + id;
}
function sportsReplayDate(item) {
  const value = item && (item.release_date || item.first_air_date || (item.sort_metrics || {}).release_date);
  if (!value) return "Sports replay";
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? "Sports replay" : date.toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" });
}
function renderSportsReplays(event) {
  const matches = sportsReplayMatchesForEvent(event).slice(0, 2);
  if (!matches.length) return "";
  return "<div class=\"sports-replays\"><span class=\"sports-replays-label\">Replays</span>" + matches.map(function(match) {
    const item = match.item || {};
    return "<a class=\"sports-replay\" href=\"" + escapeHTML(sportsReplayHref(item)) + "\"><span>" + icon("play") + "</span><span><strong data-overflow-tooltip=\"" + escapeHTML(item.title || "Sports replay") + "\">" + escapeHTML(item.title || "Sports replay") + "</strong><small>" + escapeHTML(sportsReplayDate(item)) + "</small></span><b>Watch Replay</b></a>";
  }).join("") + "</div>";
}
function uniqueEventChannels(channels) {
  ...
  return items(channels).filter(function(channel) {
    ...
    if (lower(channel && channel.confidence) === "low") return false;
...
function playerSportsEventChannel(event) {
  const currentID = state.currentChannel && state.currentChannel.id;
  const channels = rankedSportsBroadcasts(event || {}).filter(function(channel) { return Number(channel.score || 0) >= 60; });
...
function playerSportsCurrentEvent(events) {
...
function renderPlayerSportsFeed(channel) {
  ...
  const current = currentProgram(appChannel) || {};
  const next = nextProgram(appChannel) || {};
  const active = String(channel.id || "") === String(state.currentChannel && state.currentChannel.id || "");
...
function renderPlayerSportsDrawer() {
  ...
  const loading = !events.length && (!state.sports || state.sportsLoading || !!state.sports.refreshing);
  ...
  if (root.playerSportsMarkup === markup) return;
  const body = root.querySelector(".player-sports-body");
  const scrollTop = body ? body.scrollTop : 0;
  root.innerHTML = markup;
  root.playerSportsMarkup = markup;
  root.querySelector(".player-sports-body").scrollTop = scrollTop;
  observePlayerSportsDrawerLayout();
}
...
function togglePlayerSports(open) {
  if (!sportsFirstPlayerActive()) return;
...
function setSportsTab(tab) {
  state.sportsTab = sportsHubTab(tab);
  state.sportsTeam = "";
  state.sportsLeague = "";
...
function backFromSportsDetail() {
  ...
  if (state.sportsSelectedEventID) {
    state.sportsSelectedEventID = "";
  } else if (state.sportsTeam) {
    state.sportsTeam = "";
  } else {
    state.sportsLeague = "";
    state.sportsTab = "today";
  }
  renderSportsPage();
}
...
function eventTabLabel(tab) {
  return ({ featured: "Featured", live: "Live", upcoming: "Upcoming", all: "All" })[tab] || "Live";
}
function eventTabButtonsHTML() {
  return ["featured", "upcoming", "live", "all"].map(function(tab) {
...
function renderEventsPage() {
  ...
  const events = collapseRepeatAirings(filteredBroadcastEvents(payload));
  root.innerHTML = "<div class=\"sports-page\"><div class=\"sports-pinned\"><div class=\"event-filter-bar\">" + renderEventTabFilters() + renderEventCategoryFilters(payload) + "</div>"
...
function filteredBroadcastEvents(payload) {
  const now = Math.floor(Date.now() / 1000);
  const personal = featuredEventMap();
  const global = adminFeaturedEventMap();
  return items(payload && payload.events).filter(function(event) {
    if (!uniqueEventChannels(event.channels).length) return false;
    if (state.eventCategory && event.categoryId !== state.eventCategory) return false;
    const live = sportsEventIsLive(event);
    if (state.eventsTab === "featured" && !global[event.id] && !personal[event.id]) return false;
    ...
    const leftPriority = global[left.id] ? 4 : (personal[left.id] ? 3 : (sportsEventIsLive(left) ? 2 : 1));
    const rightPriority = global[right.id] ? 4 : (personal[right.id] ? 3 : (sportsEventIsLive(right) ? 2 : 1));
    if (leftPriority !== rightPriority) return rightPriority - leftPriority;
    return Number(left.startUnix || Number.MAX_SAFE_INTEGER) - Number(right.startUnix || Number.MAX_SAFE_INTEGER);
  });
}
function collapseRepeatAirings(events) {
  const byKey = {};
  const result = [];
  items(events).forEach(function(event) {
    const channel = uniqueEventChannels(event.channels)[0];
    const key = lower(event.name || event.shortName) + "|" + (channel ? channel.id : "") + "|" + (event.completed ? "ended" : "open");
    const first = byKey[key];
    if (first && !sportsEventIsLive(event)) {
      first.laterAirings.push(Number(event.startUnix || 0));
      return;
    }
    const copy = Object.assign({}, event, { laterAirings: [] });
    if (!first) byKey[key] = copy;
    result.push(copy);
  });
  return result;
}
...
function eventArtworkLoaded(image) {
  if (image && image.naturalWidth <= 2) eventArtworkFailed(image);
}
function eventArtworkFailed(image) {
...
function renderBroadcastEventCard(event) {
  ...
  const personallyFeatured = !!featuredEventMap()[event.id];
  const featured = globallyFeatured || personallyFeatured;
  const featureLabel = globallyFeatured ? "Featured by admin" : (personallyFeatured ? "Remove from featured" : "Feature event");
...
function toggleFeaturedEvent(eventID) {
  eventID = String(eventID || "").trim();
  if (!eventID || adminFeaturedEventMap()[eventID]) return;
  const featured = Object.assign({}, featuredEventMap());
  if (featured[eventID]) delete featured[eventID];
...
function favoriteCards(channels, hasFavorites) {
  ...
  return "<div class=\"favorites-grid\">" + channels.map(function(channel, index) {
    ...
    const controls = favoriteMap()[channel.id] ? "<div class=\"favorite-card-actions\" aria-label=\"Manage " + escapeHTML(channel.name || "channel") + "\"><button type=\"button\" class=\"favorite-remove\" data-favorite-remove=\"" + escapeHTML(channel.id) + "\" aria-label=\"Remove " + escapeHTML(channel.name || "channel") + " from favorites\" title=\"Remove from favorites\">" + icon("heart-solid") + "</button>" + reorder + "</div>" : "";
...
function categoryPrefixSplit(name) {
  ...
  return match ? { prefix: match[1], rest: match[2].trim() } : null;
}
function categoryPrefixGroups(categories) {
  const byPrefix = {};
  const order = [];
  (categories || []).forEach(function(category) {
    const split = categoryPrefixSplit(category && (category.name || category.id));
    if (!split) return;
    const key = split.prefix.toLowerCase();
    if (!byPrefix[key]) { byPrefix[key] = { prefix: split.prefix, categories: [], names: {} }; order.push(key); }
    byPrefix[key].categories.push(category);
    byPrefix[key].names[category.id] = split.rest;
  });
  const groups = order.map(function(key) { return byPrefix[key]; }).filter(function(group) { return group.categories.length >= 2; });
  const groupedIDs = {};
  groups.forEach(function(group) { group.categories.forEach(function(category) { groupedIDs[category.id] = true; }); });
  return { groups: groups, loose: (categories || []).filter(function(category) { return !groupedIDs[category.id]; }) };
}
  ...
  if (state.category.indexOf("virtual:") === 0 || state.category.indexOf("featured:") === 0) {
    ...
    const childFolders = browseOnly
      ? (filteredChildren.length
        ? "<div class=\"category-grid\" aria-label=\"Browse folders\">" + filteredChildren.map(function(category) { return categoryTileHTML(category); }).join("") + "</div>"
...
function deleteSavedLineupEditor() {
  ...
  closeSavedLineupEditor();
  render();
  showAppToast("Removed saved lineup.");
}
...
function renderMultiviewPage() {
  resetMultiviewMedia();
  const connectionLimit = multiviewConnectionLimit();
  ...
  byId("view").innerHTML = "<section class=\"multiview-page\"><div id=\"multiview-audio-status\" class=\"sr-only\" role=\"status\"></div><div class=\"multiview-toolbar\"><div><h2>Multiview</h2><p>" + escapeHTML(title) + " · focused tile owns audio" + (connectionLimit < 4 ? " · limited for this connection" : "") + "</p></div><div class=\"multiview-actions\"><span class=\"multiview-count\">" + escapeHTML(String(tiles.length)) + "/" + escapeHTML(String(connectionLimit)) + "</span>" + (tiles.length ? "<button class=\"chip\" type=\"button\" data-multiview-action=\"clear\">Clear</button>" : "") + "</div></div>"
    + (tiles.length ? "<div class=\"multiview-grid " + countClass + "\">" + tiles.map(renderMultiviewTile).join("") + "</div>" : renderMultiviewEmpty())
    + (tiles.length && tiles.length < connectionLimit ? renderMultiviewPicker() : "")
    + "</section>";
...
function renderMultiviewEmpty() {
  return "<div class=\"multiview-empty\"><div class=\"empty\">Add up to " + escapeHTML(String(multiviewConnectionLimit())) + " live channels. The active tile is the only one with audio.</div>" + renderMultiviewPicker() + "</div>";
}
function multiviewConnectionLimit() {
  const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
  if (!connection) return 4;
  if (connection.saveData || /(^|-)2g$/.test(String(connection.effectiveType || ""))) return 1;
  if (String(connection.effectiveType || "") === "3g") return 2;
  return 4;
}
...
function addChannelToMultiview(channel) {
  ...
  const connectionLimit = multiviewConnectionLimit();
  if (state.multiviewTiles.length >= connectionLimit) {
    showAppToast(connectionLimit < 4 ? "This connection is limited to " + connectionLimit + " multiview channel" + (connectionLimit === 1 ? "." : "s.") : "Multiview supports up to four channels.");
    state.view = "multiview";
    render();
    return;
  }
...
function updateAdminSportsField(field, target) {
  ...
  if (field === "library") {
    ...
    if (target.checked && libraryIDs.indexOf(libraryID) === -1) libraryIDs.push(libraryID);
    if (!target.checked) libraryIDs = libraryIDs.filter(function(id) { return id !== libraryID; });
    settings.sportsLibraryIds = libraryIDs;
  }
  state.adminCategorySettings = settings;
  normalizeAdminCategorySettings();
  state.sportsReplayKey = "";
...
function renderAdminPage() {
  ...
  if (state.adminTab === "sports") {
    renderAdminSportsSettings();
  }
  ...
  if (state.adminTab === "events") {
    renderAdminFeaturedEventSettings();
...
function renderAdminSettingsTab(tab) {
  ...
  if (tab === "events") {
    return "<div class=\"settings-card\"><div class=\"settings-card-head\"><div><h2>Featured events</h2><p>Pin important guide events above automatic recommendations for everyone.</p></div></div><div id=\"admin-featured-event-settings\" class=\"settings-list admin-featured-event-list\"></div></div>"
...
function renderAdminSportsSettings() {
  ...
  const settings = adminSettings();
  const selected = {};
  configuredSportsLibraryIDs().forEach(function(id) { selected[id] = true; });
  let libraryHTML = "";
  if (state.sportsLibrariesLoading) {
    libraryHTML = "<div class=\"settings-note\">Loading accessible Silo libraries...</div>";
  } else if (state.sportsLibrariesError) {
    libraryHTML = "<div class=\"settings-note settings-warning settings-note-action\"><span>Could not load Silo libraries: " + escapeHTML(state.sportsLibrariesError) + "</span><button type=\"button\" data-admin-sports-action=\"refresh-libraries\">Retry</button></div>";
  } else if (!items(state.sportsLibraries).length) {
    libraryHTML = "<div class=\"settings-note\">No accessible Silo libraries are available.</div>";
  } else {
    const visible = {};
    items(state.sportsLibraries).forEach(function(library) { visible[Number(library.id)] = true; });
    const unavailable = configuredSportsLibraryIDs().filter(function(id) { return !visible[id]; }).length;
    libraryHTML = (unavailable ? "<div class=\"settings-note settings-warning\">" + unavailable + " configured " + (unavailable === 1 ? "library is" : "libraries are") + " not accessible to this admin account and will remain configured.</div>" : "")
      + "<div class=\"profile-selection-list sports-library-list\">" + items(state.sportsLibraries).map(function(library) {
        const id = Number(library.id);
        return "<label class=\"profile-selection-row\"><span><strong>" + escapeHTML(library.name || ("Library " + id)) + "</strong><small>" + escapeHTML(library.type || "Silo library") + "</small></span><input type=\"checkbox\" data-admin-sports-field=\"library\" data-admin-sports-library-id=\"" + id + "\"" + (selected[id] ? " checked" : "") + (settings.sportsEnabled ? "" : " disabled") + "></label>";
      }).join("") + "</div>";
  }
  root.innerHTML = adminSaveStatusHTML()
    ...
    + libraryHTML
    + renderAdminSportsTeamChannels(settings);
  if (!state.sportsLibraries && !state.sportsLibrariesLoading) loadSportsLibraries(false);
  if (!state.sports && !state.sportsLoading && settings.sportsEnabled) loadSports(false);
}
function adminSportsTeamOptions() {
  const teams = sportsHubTeams(state.sports || {});
  return Object.keys(teams).map(function(key) { return teams[key]; }).sort(function(left, right) {
    return String(left.leagueName).localeCompare(String(right.leagueName)) || sportsTeamName(left.team).localeCompare(sportsTeamName(right.team));
  });
}
function ensureAdminChannelsLoaded() {
  if (!isAdminRoute || state.adminChannelsRequested) return;
  state.adminChannelsRequested = true;
  getJSON("/dispatcharr/api/channels").then(function(payload) {
    if (state.app) state.app.channels = items(payload && payload.channels);
  }).catch(function() {}).then(function() {
    if (state.view === "admin" && state.adminTab === "sports") renderAdminSportsSettings();
  });
}
function renderAdminSportsTeamChannels(settings) {
  ensureAdminChannelsLoaded();
  const pins = items(settings.sportsTeamChannels);
  const teams = adminSportsTeamOptions();
  const channels = items(state.app && state.app.channels).slice().sort(function(left, right) { return String(left.name || "").localeCompare(String(right.name || "")); });
  const leagueName = function(id) {
    const league = sportsLeagueByID(state.sports, id);
    return (league && league.name) || id;
  };
  const list = pins.length ? "<div class=\"profile-selection-list sports-team-channel-list\">" + pins.map(function(pin, index) {
    const channel = channelByID(pin.channelId);
    return "<div class=\"profile-selection-row\"><span><strong>" + escapeHTML(pin.teamName) + "</strong><small>" + escapeHTML(leagueName(pin.leagueId) + " → " + ((channel && channel.name) || pin.channelName || pin.channelId) + (channel ? "" : " (channel not found)")) + "</small></span><button type=\"button\" data-admin-sports-action=\"remove-team-channel\" data-admin-team-channel-index=\"" + index + "\" aria-label=\"Remove channel for " + escapeHTML(pin.teamName) + "\">" + icon("x") + "</button></div>";
  }).join("") + "</div>" : "<div class=\"settings-note\">No team channels yet. Pin a team to its regional or team network so its games always get a Watch button.</div>";
  const teamOptions = "<option value=\"\">Choose a team</option>" + teams.map(function(entry) {
    return "<option value=\"" + escapeHTML(entry.leagueID + "|" + sportsTeamName(entry.team)) + "\">" + escapeHTML(sportsTeamName(entry.team) + " · " + entry.leagueName) + "</option>";
  }).join("");
  const channelOptions = "<option value=\"\">Choose a channel</option>" + channels.map(function(channel) {
    return "<option value=\"" + escapeHTML(channel.id) + "\">" + escapeHTML(channel.name || channel.id) + "</option>";
  }).join("");
  return "<div class=\"settings-card-head sports-library-head\"><div><h3>Team channels</h3><p>Pin a team to one channel, such as a regional sports network. Its live and upcoming games list that channel first.</p></div><span class=\"profile-selection-summary\">" + pins.length + " pinned</span></div>"
    + list
    + "<div class=\"sports-team-channel-form\"><label><span>Team</span><select id=\"admin-team-channel-team\"" + (teams.length ? "" : " disabled") + ">" + teamOptions + "</select></label><label><span>Channel</span><select id=\"admin-team-channel-channel\">" + channelOptions + "</select></label><button type=\"button\" data-admin-sports-action=\"add-team-channel\">Pin channel</button></div>"
    + (teams.length ? "" : "<div class=\"settings-note\">Teams appear here once Sports data has loaded.</div>");
}
function updateAdminSportsTeamChannels(action, button) {
  const settings = state.adminCategorySettings || defaultAdminCategorySettings();
  const pins = items(settings.sportsTeamChannels).slice();
  if (action === "remove-team-channel") {
    pins.splice(Number(button.getAttribute("data-admin-team-channel-index")), 1);
  } else {
    const team = String((byId("admin-team-channel-team") || {}).value || "");
    const channelID = String((byId("admin-team-channel-channel") || {}).value || "");
    const split = team.indexOf("|");
    if (split < 1 || !channelID) {
      showAppToast("Choose a team and a channel.");
      return;
    }
    const leagueId = team.slice(0, split);
    const teamName = team.slice(split + 1);
    const channel = channelByID(channelID);
    const existing = pins.findIndex(function(pin) { return pin.leagueId === leagueId && lower(pin.teamName) === lower(teamName); });
    const pin = { leagueId: leagueId, teamName: teamName, channelId: channelID, channelName: (channel && channel.name) || "" };
    if (existing >= 0) pins[existing] = pin;
    else pins.push(pin);
  }
  settings.sportsTeamChannels = pins;
  state.adminCategorySettings = settings;
  normalizeAdminCategorySettings();
  markAdminSettingsDraft();
  renderAdminPage();
...
function renderAdminFeaturedEventSettings() {
  const root = byId("admin-featured-event-settings");
  if (!root) return;
  if (!state.events && !state.eventsLoading) {
    state.eventsLoading = true;
    getJSON("/dispatcharr/api/events").then(function(payload) {
      state.events = payload || { events: [], categories: [] };
    }).catch(function(error) {
      state.events = { events: [], categories: [], error: readableError(error) };
    }).finally(function() {
      state.eventsLoading = false;
      if (state.view === "admin" && state.adminTab === "events") renderAdminPage();
    });
  }
  const selected = adminFeaturedEventMap();
  const events = items(state.events && state.events.events).filter(function(event) {
    return !event.completed || selected[event.id];
  }).sort(function(left, right) {
    if (!!selected[left.id] !== !!selected[right.id]) return selected[left.id] ? -1 : 1;
    if (sportsEventIsLive(left) !== sportsEventIsLive(right)) return sportsEventIsLive(left) ? -1 : 1;
    return Number(left.startUnix || Number.MAX_SAFE_INTEGER) - Number(right.startUnix || Number.MAX_SAFE_INTEGER);
  }).slice(0, 40);
  if (state.eventsLoading && !events.length) {
    root.innerHTML = "<div class=\"settings-note\">Loading detected events...</div>";
    return;
  }
  if (!events.length) {
    root.innerHTML = "<div class=\"settings-note" + (state.events && state.events.error ? " settings-warning" : "") + "\">" + escapeHTML((state.events && state.events.error) || "No upcoming guide events are currently detected.") + "</div>";
    return;
  }
  root.innerHTML = "<div class=\"admin-featured-event-grid\">" + events.map(function(event) {
    const checked = !!selected[event.id];
    const status = sportsEventIsLive(event) ? "Live now" : eventStatusLabel(event);
    return "<label class=\"admin-featured-event-row\"><span><strong>" + escapeHTML(event.shortName || event.name || "Event") + "</strong><small>" + escapeHTML([event.categoryName, status].filter(Boolean).join(" · ")) + "</small></span><input type=\"checkbox\" data-admin-featured-event-id=\"" + escapeHTML(event.id || "") + "\"" + (checked ? " checked" : "") + "></label>";
  }).join("") + "</div>";
}
...
document.addEventListener("click", function(event) {
  ...
  if (adminSportsAction) {
    event.preventDefault();
    const sportsAction = adminSportsAction.getAttribute("data-admin-sports-action");
    if (sportsAction === "refresh-libraries") loadSportsLibraries(true);
    if (sportsAction === "add-team-channel" || sportsAction === "remove-team-channel") updateAdminSportsTeamChannels(sportsAction, adminSportsAction);
    return;
  }
  ...
  if (sportsRefresh) {
    event.preventDefault();
    sportsGameStatsState.fetchedAt = 0;
    loadSports(true);
    renderSportsPage();
    return;
  }
  ...
  const eventFeature = event.target.closest("[data-event-feature]");
  if (eventFeature) {
    event.preventDefault();
    toggleFeaturedEvent(eventFeature.getAttribute("data-event-feature"));
    return;
  }
  ...
  if (favoriteRemove) {
    event.preventDefault();
    setChannelFavorite(favoriteRemove.getAttribute("data-favorite-remove"), false);
    render();
    return;
  }
...
document.addEventListener("keydown", function(event) {
  ...
  if (event.key === "ArrowDown" && sportsFirstPlayerActive() && !state.playerSportsOpen) {
    event.preventDefault();
...
document.addEventListener("change", function(event) {
  ...
  const adminSportsField = event.target.getAttribute("data-admin-sports-field");
  if (adminSportsField) {
    updateAdminSportsField(adminSportsField, event.target);
    return;
  }
...
document.addEventListener("input", function(event) {
  ...
  const adminFeaturedEventID = event.target.getAttribute("data-admin-featured-event-id");
  if (adminFeaturedEventID) {
    const settings = state.adminCategorySettings || defaultAdminCategorySettings();
    const selected = {};
    items(settings.featuredEventIds).forEach(function(id) { selected[String(id || "")] = true; });
    if (event.target.checked) selected[adminFeaturedEventID] = true;
    else delete selected[adminFeaturedEventID];
    settings.featuredEventIds = Object.keys(selected);
    state.adminCategorySettings = settings;
    markAdminSettingsDraft();
    renderAdminTopbarActions();
    return;
