...
func TestSportsScoresVisibleWithStaleProviderStatus(t *testing.T) {
	...
	result := runUIInvariantScript(t, []string{
		...
		`if (safeSportsMediaURL("/dispatcharr/api/sports/image?key=example") !== "/api/v1/plugins/14/dispatcharr/api/sports/image?key=example") throw new Error("proxy images must use the plugin installation route");`,
