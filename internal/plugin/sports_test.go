...
func TestGuideSportsMatchupParsesQualifiedBroadcastTitles(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "cricket playoff is event metadata",
			title:     "Cricket Highlights : WCPL 2026: Tridents vs Warriors - Playoff",
			wantAway:  "Tridents",
			wantHome:  "Warriors",
			wantMatch: true,
		},
		{
			name:      "event channel air time is not the home team",
			title:     "DLR Waves v Athlone Town @ 19 Sep 02:30 PM ET",
			wantAway:  "DLR Waves",
			wantHome:  "Athlone Town",
			wantMatch: true,
		},
		{
			name:      "abbreviated matchup keeps both teams before the air time",
			title:     "NATIONAL 12: NYM at TEX @ 24 Sep 02:05 PM ET",
			wantAway:  "NYM",
			wantHome:  "TEX",
			wantMatch: true,
		},
		{
			name:      "uppercase air time without an at sign is stripped",
			title:     "OHL 05 | 2026 OSHAWA GENERALS vs GUELPH STORM SEP 25 06:00 PM",
			wantAway:  "Oshawa Generals",
			wantHome:  "Guelph Storm",
			wantMatch: true,
		},
		{
			name:      "time-only air suffix is stripped",
			title:     "Soccer11: Soccer: North America - CONCACAF Nations League: Bahamas vs Saint Martin @ 04:00 PM EDT",
			wantAway:  "Bahamas",
			wantHome:  "Saint Martin",
			wantMatch: true,
		},
		{
			name:      "feed tag and lowercase clock are stripped",
			title:     "Soccer08: Korea DPR U20 vs Colombia U20 ( TSN3 Feed ) @ 12:30 pm",
			wantAway:  "Korea DPR U20",
			wantHome:  "Colombia U20",
			wantMatch: true,
		},
		{
			name:      "stage prefix is not part of the away team",
			title:     "Super League Rugby: Semi-Final - Leeds Rhinos v Warrington Wolves",
			wantAway:  "Leeds Rhinos",
			wantHome:  "Warrington Wolves",
			wantMatch: true,
		},
		{
			name:      "secplus channel tag is dropped and ranks stay",
			title:     "SEC+ 12: #3 Vanderbilt vs. Ole Miss (SECPLUS)",
			wantAway:  "#3 Vanderbilt",
			wantHome:  "Ole Miss",
			wantMatch: true,
		},
		{
			name:      "shouted names keep short abbreviations",
			title:     "MMA 03: MUSUMECI vs MITCHELL (09.24 08:00 PM ET)",
			wantAway:  "Musumeci",
			wantHome:  "Mitchell",
			wantMatch: true,
		},
		{
			name:      "network tag and air time are not part of the home team",
			title:     "ACCNX 07: Louisville vs. California (ACCNX) @ 25 Sep 05:00 PM ET",
			wantAway:  "Louisville",
			wantHome:  "California",
			wantMatch: true,
		},
		{
			name:      "pipe suffix after the home team is broadcast metadata",
			title:     "Leeds Rhinos v Warrington Wolves | Super League @ 25 Sep 08:00 PM ET",
			wantAway:  "Leeds Rhinos",
			wantHome:  "Warrington Wolves",
			wantMatch: true,
		},
		...
		{
			name:      "dotted parenthetical air time is stripped",
			title:     "SEC+ ACCNX 005: SEC Network +: Auburn vs. Missouri (09.24 07:00 PM ET)",
			wantAway:  "Auburn",
			wantHome:  "Missouri",
			wantMatch: true,
		},
		{
			name:      "game number suffix is stripped",
			title:     "QMJHL 00: Halifax @ Baie-Comeau - #402",
			wantAway:  "Halifax",
			wantHome:  "Baie-Comeau",
			wantMatch: true,
		},
		{
			name:      "game number before a pipe schedule is stripped",
			title:     "QMJHL 11: Cape Breton @ Quebec - #519 | Fri 7th 12:00AM",
			wantAway:  "Cape Breton",
			wantHome:  "Quebec",
			wantMatch: true,
		},
		{
			name:      "multi-part games suffix is stripped",
			title:     "Pakistan vs Uzbekistan - Hockey Men's  - Asian Games Aichi-Nagoya 2026",
			wantAway:  "Pakistan",
			wantHome:  "Uzbekistan",
			wantMatch: true,
		},
		...
		{
			name:      "studio show with an air time is not a matchup",
			title:     "TSN+ 10: Formula 1 Pit Lane - Azerbaijan Grand Prix Practice #2 @ 24 Sep 07:50 AM ET",
			wantMatch: false,
		},
		{
