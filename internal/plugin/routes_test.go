...
func TestPlayerAppApprovedUXPassContracts(t *testing.T) {
	...
	for _, want := range []string{`aria-label="Watch"`, `Event coverage`, `renderSportsBroadcastGroups`, `renderSportsCoverageCard`, `rankedSportsBroadcasts`, `renderSportsEventNavigation`, `data-sports-spoilers`, `data-sports-favorite-league`} {
	...
	for _, want := range []string{
		...
		`.shell.is-sports #view { min-height: 0; overflow-x: hidden; overflow-y: auto;`,
		`.shell.is-events .main { overflow: auto; }`,
		`.shell.is-events #view { min-height: 0; overflow: visible;`,
		`.guide-window-spacer`,
		`.guide-window`,
		`.recovery-panel`,
		`.filter-sections`,
		...
		`.event-shelf-rail { grid-auto-flow: row; grid-template-columns: repeat(auto-fill, minmax(min(18rem, 100%), 1fr));`,
		...
		`.sports-league-grid {`,
		`.sports-event-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); align-items: start;`,
