...
func TestManifestDeclaresPublicApplicationRoutesOnly(t *testing.T) {
	...
	for _, route := range []string{
		...
		"POST /dispatcharr/api/timeshift/start",
		"GET /dispatcharr/api/timeshift/status",
		"POST /dispatcharr/api/timeshift/heartbeat",
...
func TestHTTPRoutesServerAdminPageIncludesCategoryMapping(t *testing.T) {
	...
	for _, want := range []string{
		...
		`data-admin-organization-tab=\"rules\"`,
		`data-admin-organization-filter`,
		`organization-control-row`,
		`organization-bulk-menu`,
		...
		`Choose what appears in Live TV, then control how those names become folders.`,
		`Enable all`,
		`Folder rules`,
		...
		`data-admin-tab=\"general\"`,
		`data-admin-tab=\"playback\"`,
		`data-admin-tab=\"organization\"`,
		`data-admin-tab=\"events\"`,
...
func TestPlayerWatchHistoryDoesNotCreateChannelFavorites(t *testing.T) {
	t.Parallel()
...
const result = vm.runInContext(`+"`"+`
  const migratedAutoFavorites = Object.keys(mergePrefs({ autoFavorites: { "channel:legacy": true } }).autoFavorites || {});
  state.app = { channels: [{ id: "channel:1", name: "Test Channel" }], preferences: defaultPrefs() };
  savePrefs = function() {};
  const channel = state.app.channels[0];
  recordWatchPreference(channel);
  recordWatchPreference(channel);
  recordWatchPreference(channel);
  ({
    favorites: Object.keys(state.app.preferences.favorites || {}),
    autoFavorites: Object.keys(state.app.preferences.autoFavorites || {}),
    migratedAutoFavorites,
    recentChannels: state.app.preferences.recentChannels,
    plays: state.app.preferences.continueWatching["channel:1"].plays,
  });
`+"`"+`, sandbox);
...
`, appScriptPath)
	...
	if err != nil {
		t.Fatalf("run watch-history regression: %v\n%s", err, output)
	}
	var result struct {
		Favorites             []string `json:"favorites"`
		AutoFavorites         []string `json:"autoFavorites"`
		MigratedAutoFavorites []string `json:"migratedAutoFavorites"`
		RecentChannels        []string `json:"recentChannels"`
		Plays                 int      `json:"plays"`
	}
	if err := json.Unmarshal(output, &result); err != nil {
		t.Fatalf("decode watch-history regression: %v\n%s", err, output)
	}
	if len(result.Favorites) != 0 || len(result.AutoFavorites) != 0 || len(result.MigratedAutoFavorites) != 0 {
		t.Fatalf("watch history must not create a favorite without an explicit user action: %+v", result)
	}
	if len(result.RecentChannels) != 1 || result.RecentChannels[0] != "channel:1" || result.Plays != 3 {
		t.Fatalf("expected watch history to remain available for recently watched: %+v", result)
	}
}
...
const sandbox = {
  window: { location: { pathname: "/api/v1/plugins/14/dispatcharr/admin", search: "" }, addEventListener: () => {}, innerHeight: 800, scrollY: 0 },
  document: { documentElement: { dataset: {} }, body: {}, querySelectorAll: () => [], querySelector: () => null, getElementById: () => null, addEventListener: () => {}, contains: () => true },
  ...
  fetch: async (url, options = {}) => {
    ...
    if (String(url) === "/api/v1/auth/refresh") return response(200, { access_token: "fresh-access", refresh_token: "fresh-refresh" });
...
vm.runInContext(source, sandbox);
(async () => {
  await vm.runInContext('corePutNoContent("/api/v1/admin/plugins/installations/14/config", { key: "category_settings", value: {} })', sandbox);
